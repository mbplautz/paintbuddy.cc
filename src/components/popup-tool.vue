<template>
    <div :class="['popup-container', name]">
        ...
        <div :class="['popup-tool', popupVisible ? 'popup-layer' : '']" :style="`top: ${heightOffset}px;${ visible ? '' : ` left: ${-width}px;`}${ initiallyHidden ? ' z-index: -5;' : ''}`" ref="popup-ref">
            <slot />
        </div>
    </div>
</template>
